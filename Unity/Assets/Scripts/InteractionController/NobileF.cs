...
public class NobileF : MonoBehaviour
{
        ...
        if (!audioSource.isPlaying)
        {
            switch (caller.tag)
            {
                case "Player_Nobile":
                    ...
                    audioSource.clip = Resources.Load<AudioClip>("Talking/Nobile_NobileF" + index.ToString());
                    audioSource.Play();
                    break;
                case "Player_Schiavo":
                    // AUdio schiavo: prendi negli asset la stringa corrispondente a "Schiavo_NobileM + (index)"
                    audioSource.clip = Resources.Load<AudioClip>("Talking/Schiavo_NobileF" + index.ToString());
                    break;
            }
        }
    }
